                ...
                <div className="hidden md:flex space-x-1">
                    {navItems.map((item) => (
                        <Link
                            key={item.name}
                            href={item.href}
                            className={`px-3 py-2 rounded-md text-sm font-medium text-primary-foreground hover:bg-[#5fd0c7] hover:text-black transition-all duration-300 ease-in-out transform hover:scale-105 ${pathname === item.href ? "bg-secondary text-secondary-foreground" : ""
                                }`}
                        >
                            {item.name}
                        </Link>
                ...
                <div className="md:hidden mt-2 space-y-2">
                    {navItems.map((item) => (
                        <Link
                            key={item.name}
                            href={item.href}
                            className={`block py-2 px-4 text-primary-foreground hover:bg-[#5fd0c7] hover:text-black transition-all duration-300 rounded-md ${pathname === item.href ? "bg-secondary text-secondary-foreground" : ""
                                }`}
